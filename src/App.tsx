  ...
  return (
    <ThemeProvider defaultTheme="dark" storageKey="lifesync-theme">
      <div className="min-h-screen bg-slate-50 dark:bg-[#0D0F14] flex font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
        ...
        <aside
          className={cn(
            "fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-[#151821] border-r border-slate-200 dark:border-slate-800 transform transition-all duration-300 ease-in-out",
            !isSidebarOpen ? "-translate-x-full" : "translate-x-0"
          )}
        >
          <button
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3 w-full hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group"
          >
            <div className="size-8 bg-blue-600 rounded-lg flex items-center justify-center text-white shadow-lg shadow-blue-900/20 group-hover:shadow-blue-900/40 transition-shadow">
        ...
        <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden bg-slate-50 dark:bg-[#0D0F14]">
          <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#151821] flex items-center justify-between px-6 lg:px-10 shrink-0 transition-all">
            <div className="flex items-center gap-3">
              {/* Menu Toggle Button */}
              {!isSidebarOpen && (
                <button
                  onClick={() => setIsSidebarOpen(true)}
                  className="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md hover:shadow-lg transition-all"
